// server.js (TÜM ÖZELLİKLERİ BARINDIRAN FİNAL SÜRÜM)
...
const app = express();
const PORT = process.env.PORT || 8000; // Render gibi servislerin verdiği portu kullanır
...
function loadSettings(callback) {
    db.all("SELECT setting_key, setting_value FROM settings", [], (err, rows) => {
        if (err) {
            console.error("AYARLAR YÜKLENEMEDİ:", err.message);
            return callback(err);
        }
        rows.forEach(row => {
...
async function getMsGraphToken() {
    ...
    if (!settings.tenant_id || !settings.client_id || !settings.client_secret) {
        console.log("Azure ayarları eksik, token alınamıyor.");
        return null;
    }
...
async function getLatestEmail() {
    ...
    } catch (error) { const errorMessage = error.response?.data?.error?.message || error.message; return { error: `Mail çekilemedi: ${errorMessage}` }; }
}

// --- Express Ayarları ---
app.set('view engine', 'ejs');
app.use(express.urlencoded({ extended: true }));
app.use(session({ secret: 'klavyemden-cıkan-cok-gizli-kelimeler', resave: false, saveUninitialized: true, }));
